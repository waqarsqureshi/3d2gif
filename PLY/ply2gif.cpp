// version 1.0 : initial version of PLY to GIF
...
// Global variables
std::vector<vertex2> vertices;    // for PLY access, vertices
std::vector<Vid3> facets;         // for PLY access, facet indices
std::vector<texture> textures;    // for PLY access, texture coordinates
...
int main(int argc, char **argv) {
    ...
    if ( argc == 1) {
        ...
        char *sizePLY_ = getCmdOption(argv, argv + argc, "-s");    //   object-over-image portion, default is maximum bounding box by not provide this option
        ...
        // (!sizePLY_) ? sizePLY = 4.0 : sizePLY = stod(sizePLY_);
        (!angleX) ? angX =   0.0 : angX = stoi(angleX);
        (!angleY) ? angY = 180.0 : angY = stoi(angleY);
        (!angleZ) ? angZ = 180.0 : angZ = stoi(angleZ);
        (!rx_) ? rx = 0.0 : rx = stod(rx_);
        (!ry_) ? ry = 1.0 : ry = stod(ry_);
        (!rz_) ? rz = 0.0 : rz = stod(rz_);

        MyMesh mesh;
        string plyName, pngName;
        filePath inputFile(fileName);
        folderPath = inputFile.path;

        plyName = inputFile.path + inputFile.name;
        err = vcg::tri::io::ImporterPLY<MyMesh>::Open(mesh,plyName.c_str()); // read PLY
	cout<<"Error = "<<err<<endl;//waqar
        cout<<"mesh.texture size = "<<mesh.textures.size()<<endl;//waqar	
        if (mesh.textures.size()>0)
                HasPNG = true;

        vcg::tri::io::ExporterPLY<MyMesh>::getPlyInfo(mesh);            // extract PLY information, faces, vertices, texcoord
        ...
        (!sMul_) ? sMul = 1.7 : sMul = stod(sMul_);  // default of -s is approximately 1.7 * max-z
        (!sizePLY_) ? sizePLY = sMul*bbox_max : sizePLY = stod(sizePLY_);
...
void display(void) {
        ...
        float min_z = bbox_max/10000; // min-z
...
void help() {
    ...
    std::cout << "./gifPLY -i /fullpath/to/PLY\n" << std::endl;
    std::cout << "-i: *required fullpath of PLY object" << std::endl;
